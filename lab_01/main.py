...
def main():
	...
	result_mtr = matrix(word1, word2)
	print('result matrix: ', result_mtr)
	mtr = demerau_lev(word1, word2)
	print('demerau_lev: ', mtr)
	start = time.time()
...
def recursion(word1, word2):
    ...
    result = min([recursion(word1[:-1], word2) + 1, recursion(word1, word2[:-1]) + 1, recursion(word1[:-1], word2[:-1]) + flagmatch])
